...
csfle_opts = AutoEncryptionOpts(
   kms_providers=kms_providers, key_vault_namespace="cs_lab7.__keystore"
)
# Connect to MongoDB with the key information generated above:
with MongoClient("mongodb+srv://CostelLab7:LaboratoryWorkCS@cluster0.wzmuw.mongodb.net/Cluster0?retryWrites=true&w=majority", auto_encryption_opts=csfle_opts) as client:
    print("Resetting demo database & keystore ...")
    client.drop_database("cs_lab7")
...
schema = {
  ...
  "properties": {
      "real_name": {
          "encrypt": {
              "bsonType": "string",
              # Change to "AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic" in order to filter by real name value:
      ...
      "last_known_location": {
          "encrypt": {
              "bsonType": "string",
              # Change to "AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic" in order to filter by last known location value:
              "algorithm": "AEAD_AES_256_CBC_HMAC_SHA_512-Random",
              "keyId": [key_id],  # Reference the key
          }
      },
