...
root = Tk()
root.title('Lab Work No. 2 - Constantin Cazacu')
...
def save():
    ...
    def check():
        ...
        if extension == 'audit':
            text_file = filedialog.asksaveasfilename(defaultextension='.*', initialdir='Lab_2/Policies/',
        ...
        if text_file:
            ...
            name = text_file
            status_bar.config(text=f'{name}       ')
            name = name.replace('Lab_2/Policies/', '')
            root.title(f'{name} - Constantin Cazacu')
